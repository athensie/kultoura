...
analytics_track($conn, 'accommodation');

$accommodationHeroPhoto = sitecontent_get_photo($conn, 'accommodation_hero');

// Logged for foryou.php's "Because You Explored" recommendations.
$_SESSION['history'][] = 'accommodation';
...
$stmt = $conn->prepare(
    "SELECT d.*,
     ...
     FROM destination d
     WHERE d.category = 'accommodation' AND d.status = 'active'
...
<head>
    ...
    <title>Accommodation – KULTOURA</title>
...
<!-- ── Page Hero ── -->
<section class="t-hero<?php echo $accommodationHeroPhoto ? ' t-hero-has-photo' : ''; ?>">
    <?php if ($accommodationHeroPhoto): ?>
        <img class="t-hero-photo" src="<?php echo htmlspecialchars($accommodationHeroPhoto); ?>" alt="">
    ...
    <div class="t-hero-inner">
        <p class="t-eyebrow">STAY IN MALVAR · MALVAR, BATANGAS</p>
        <h1 class="t-page-title">Accommodation</h1>
        <p class="t-page-sub">
            Hotels, inns, and homestays for travelers looking to stay a little longer in Malvar.
        </p>
    </div>
</section>
...
<main class="t-main">
    ...
    <section class="t-category" id="accommodation">
        <div class="t-category-header">
            <div>
                <p class="t-cat-label">Stay in Malvar</p>
                <h2 class="t-cat-title">Accommodation</h2>
                <p class="t-cat-desc">Places to rest, from budget-friendly inns to family-run homestays.</p>
            </div>
        </div>

        <?php if (empty($spots)): ?>
        <div class="p-empty-state">
            <h3>No accommodation listed yet</h3>
...
function ktOpenViewDetails(btn) {
    ktTrackItemView('accommodation', btn.dataset.id);
