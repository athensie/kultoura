...
$stmt = $conn->prepare(
    "SELECT d.*,
     ...
     FROM destination d
     WHERE d.category = 'industry' AND d.status = 'active'
...
<head>
    ...
    <title>Industry Zone – KULTOURA</title>
...
<section class="t-hero">
    ...
    <div class="t-hero-inner">
        ...
        <p class="t-page-sub">
            Discover the commercial and industrial districts that drive Malvar forward.
        </p>
    </div>
</section>
...
<main class="t-main">
    ...
    <section class="p-search-row">
        <form class="p-search-bar" action="industry.php" method="get">
            <input type="text" name="q" placeholder="Search industrial parks, factories, districts…">
    ...
    <section class="t-category" id="industry">
        <div class="t-category-header">
            <div>
                <p class="t-cat-label">Local Destinations</p>
                <h2 class="t-cat-title">Industry Zone</h2>
                <p class="t-cat-desc">Commercial and industrial districts shaping Malvar's economy.</p>
            </div>
        </div>

        <?php if (empty($zones)): ?>
        <div class="p-empty-state">
            <h3>No industry zone entries yet</h3>
...
function ktOpenViewDetails(btn) {
    ktTrackItemView('industry', btn.dataset.id);
